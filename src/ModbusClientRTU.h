...
public:
  ...
  explicit ModbusClientRTU(Stream& serial, int8_t rtsPin = -1, uint16_t queueLimit = 100);

  // Alternative Constructor takes Serial reference and RTS line toggle callback
  explicit ModbusClientRTU(Stream& serial, RTScallback rts, uint16_t queueLimit = 100);
